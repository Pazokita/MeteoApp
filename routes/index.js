...
router.post('/add-city', async function (req, res, next) {
  const data = request("GET", `https://api.openweathermap.org/data/2.5/weather?q=${req.body.newcity}&units=metric&lang=fr&appid=${api_key}`)
...
router.get('/update-cities', async function (req, res, next) {
  ...
  for (let i = 0; i < cityList.length; i++) {
    const data = request("GET", `https://api.openweathermap.org/data/2.5/weather?q=${cityList[i].name}&units=metric&lang=fr&appid=${api_key}`)
    const dataAPI = JSON.parse(data.body)
